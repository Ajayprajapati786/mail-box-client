...
const Login = () => {
  ...
  const ForgotPasswordHandler = ()=>{
    alert("you may have received an email with reset link");
     ...
     {
      fetch(
        "https://identitytoolkit.googleapis.com/v1/accounts:sendOobCode?key=AIzaSyD5EPwVz5aGXrpKxkEQz5OqNgl7azMvfT0",
        {
          method: "POST",
          body: JSON.stringify({
  ...
  const handleSubmit = (event) => {
    ...
    if (loginId === "" || password === "") {
      ...
      {
        fetch(
          "https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=AIzaSyD5EPwVz5aGXrpKxkEQz5OqNgl7azMvfT0",
          {
            method: "POST",
            body: JSON.stringify({
              email: loginId,
